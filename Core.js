...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    ©️ *${global.BotName}* All Rights Reserved by: *Karu*
    `
...
case 'swm': case 'take': case 'stickerwm': case 'steal':{
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
if (!args.join(" ")) return reply(`Use command: -steal Miku|By: Karu`)
const swn = args.join(" ")
...
case 'handsomecheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
				if (!text) return replay(`Tag Someone, Example : ${prefix + command} @Karu`)
